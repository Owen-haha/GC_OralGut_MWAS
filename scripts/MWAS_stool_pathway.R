...
pathway <- fread("../data/Cohort1_stool_pathway_dt.tsv")
...
pathway_kept <- pathway_summ[!(prevalence<10)] # 493 pathways left
pathway1 <- pathway[pathway %in% pathway_kept$pathway]
...
ggsave("../results/MWAS_stool_pathway_res_pie.pdf", p_venn, width = 2.6*1.2, height = 2.8*1.6, units = c("in"), limitsize = FALSE)

### validation in Cohort2
pathway_v <- fread("../data/Cohort2_stool_pathway_dt.tsv")
...
fwrite(res[order(P_wilcoxon)], "../results/MWAS_stool_pathway_res.tsv", quote = F, sep = "\t", na = "-")
